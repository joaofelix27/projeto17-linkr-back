import {  getPostsByHashtag, getTrendingHashtags, insertHashtag, matchHashtag } from "../repositories/hashtagRepository.js";
...
export async function createHashtag(req, res) {
  ...
  let created= false
  try {
    await Promise.all(hashtags.map(async (value) => {
